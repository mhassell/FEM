...
*/
	...
	FirstNode.reshape(nElts,3);

	for(int i = 0; i < nElts; i++){
		for(int j = 0; j < 3; j++){
			if(FirstNode(i,j) == elements[i][j]){
				orientation[i][j] = 1;
			}
			else{
				orientation[i][j] = -1;
...
void CTriangulation::getEdgeByElement(){
...
void CTriangulation::RedRefinement(){
